...
--PMMM Mod Temp table to pass data between LuaEvents
local ttable = {}
--PMMM Mod
...
if (isBNW) then
  ...
  Controls.GK_RecommendedActionTitle:SetHide(true)
  Controls.GK_RecommendedActionLabel:SetHide(true)
else
  Controls.BNW_RecommendedActionTitle:SetHide(true)
  Controls.BNW_RecommendedActionLabel:SetHide(true)
end
...
if (isBNW) then
					...
					Controls.BNW_RecommendedActionLabel:SetText( foo .. "[NEWLINE]" .. convertedKey );
else
					Controls.GK_RecommendedActionLabel:SetText( convertedKey );
end
				end
  ...
  if (build.Condition == nil or
	...
	if (recommendedBuild == nil and 
	   ((type(build.Recommended) == "function" and build.Recommended(build, unit)) or
		...
		Controls.BNW_RecommendedActionLabel:SetText(Locale.ConvertTextKey(build.Title))
		Controls.GK_RecommendedActionLabel:SetText(Locale.ConvertTextKey(build.Title))
	end
  end
end
--
...
--
    ...
    if numBuildActions > 0 or hasPromotion then
		...
		if hasPromotion then
			...
			--PMMM Mod
			if UI.GetHeadSelectedUnit():GetUnitType() == GameInfoTypes.UNIT_PMMM_MAGICAL_GIRL then
...
function UpdateUnitPortrait( unit )
	...
	local name;
	if(isBNW and unit:IsGreatPerson()) or unit:GetUnitClassType() == GameInfoTypes.UNITCLASS_PMMM_MAGICAL_GIRL or unit:GetUnitClassType() == GameInfoTypes.UNITCLASS_PMMM_WITCH then
...
if (isBNW) then
	...
	elseif unit:GetUnitClassType() == GameInfoTypes.UNITCLASS_PMMM_MAGICAL_GIRL then
	    Controls.UnitRangedAttackBox:SetHide(false);
		LuaEvents.PMMMGetSoulGemCorruption(iPlayer, iUnit, ttable)
		local iCorruption = ttable[0]
		LuaEvents.PMMMGetSoulGemPolish(iPlayer, iUnit, ttable)
		local iPolish = ttable[0]
		LuaEvents.PMMMGetSoulGemStateStringAndDanger(iPlayer, iUnit, ttable)
		local sStateString, bCanBecomeWitch = ttable[0], ttable[1]
	...
	elseif unit:IsCombatUnit() then
		local bIsStolenFood;
		LuaEvents.PMMMGetLeaderMaxStolenFood(iPlayer, ttable)
		if ttable[0] and ttable[0] > 0 then
			bIsStolenFood = true
			if unit:GetUnitCombatType() == GameInfoTypes.UNITCOMBAT_MELEE or unit:GetUnitCombatType() == GameInfoTypes.UNITCOMBAT_GUN or unit:GetUnitCombatType() == GameInfoTypes.UNITCOMBAT_MOUNTED or unit:GetUnitCombatType() == GameInfoTypes.UNITCOMBAT_NAVALMELEE or unit:GetUnitCombatType() == GameInfoTypes.UNITCOMBAT_ARMOR then
				Controls.UnitRangedAttackBox:SetHide(false);
				local iStolenFoodMax = ttable[0]
				LuaEvents.PMMMGetUnitStolenFood(iPlayer, iUnit, ttable)
				local iStolenFood = ttable[0]
		...
		if not bIsStolenFood then
			LuaEvents.PMMMGetUnitHarmony(iPlayer, iUnit, ttable)
			if ttable[0] and ttable[0] > 0 then
				local iHarmony = ttable[0]
				Controls.UnitRangedAttackBox:SetHide(false);
...
function OnUnitRangedAttackBoxClicked()
	...
	if pUnit:GetMoves() > 0 and Players[iPlayer]:IsTurnActive() then
		--Only if the selected unit is an MG
		if pUnit:GetUnitClassType() == GameInfoTypes.UNITCLASS_PMMM_MAGICAL_GIRL then
			LuaEvents.PMMMUseGriefSeed(iUnit, iPlayer)
