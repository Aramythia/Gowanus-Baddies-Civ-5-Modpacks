-- Edited by [SSU] yenyang for MEC - Wood Elf Trait
-------------------------------------------------------------------------------
-------------------------------------------------------------------------------
function BuildTradeRouteGoldToolTipString (pOriginCity, pTargetCity, eDomain)
	...
	--Begin edit by yenyang
	local woodElfCivilizationID = GameInfoTypes["CIVILIZATION_MEC_WOOD_ELVES"];
	if (iRiverModifier ~= 0) and pPlayer:GetCivilizationType() == woodElfCivilizationID then
		iRiverModifier = iRiverModifier * 2.0;
	end
	--end edit by yenyang
